package ZoriekCaio.maratonajava.javacore.Kenum.test;

import ZoriekCaio.maratonajava.javacore.Kenum.dominio.Cliente;
import ZoriekCaio.maratonajava.javacore.Kenum.dominio.TipoCliente;

public class ClienteTest01 {
    public static void main(String[] args) {

       Cliente cliente = new Cliente("Caio", TipoCliente.PESSOA_FISICA);
       Cliente cliente2 = new Cliente("João", TipoCliente.PESSOA_JURIDICA);
       Cliente cliente3 = new Cliente("João", TipoCliente.PESSOA_FISICA);
       Cliente cliente4 = new Cliente("João", TipoCliente.PESSOA_FISICA);

        System.out.println(cliente);
        System.out.println(cliente2);
        System.out.println(cliente3);
        System.out.println(cliente4);
    }
}
